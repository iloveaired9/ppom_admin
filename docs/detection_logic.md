...
| **Kakao AdFit** | `iframe[src*="kakao_ad"]`, `*.ppomppu.co.kr/banner/kakao_ad`, `ins.kakao_ad_area` | 노란색 (#ffcc00) |
...
- **패턴 1 (iframe)**: `kakao_ad_(\d+x\d+)`
- **패턴 2 (ins)**: `data-ad-width` 및 `data-ad-height` 속성
- **결과**: `Kakao AdFit [Unit: DAN-...] 300x250` 등
...
### 2.4. Google 광고 등록 사이즈 (GPT defineSlot) 추출
스크립트 태그 내에 정의된 실제 광고 허용 사이즈 정보를 추출합니다.
- **패턴**: `googletag.defineSlot` 내의 사이즈 배열 (`[[w, h], ...]`)
- **결과**: `r_banner_f_1 [GPT: [[200, 600],[160, 600]]]` 등

### 2.5. HTTP 비정상 링크 감지
- **감지 패턴**: `http://`로 시작하거나 `http:` 뒤에 슬래시가 누락된 링크, 혹은 앞에 공백이 포함된 링크.
- **하이라이트**: 빨간색 테두리와 반투명 배경색이 적용된 하이라이트 레이어 사용.
